...
// constructing the request in restAssured and storing the response into response object
		response = given().contentType(ContentType.JSON).accept(ContentType.JSON).body(requestBody).when().post(URL);
	...
	public void i_perform_post_request_to_url_with_request_body() {
		response = given().contentType(ContentType.JSON).accept(ContentType.JSON).body(requestBody).when()
				.post(requestURL);
	}
	...
	public void response_body_match_the_request_body() {
		...
		response.then().assertThat().statusCode(200).and().assertThat().contentType("application/json").and()
				.assertThat().body("id", equalTo(667788)).and().assertThat().body("name", equalTo("Fluffy")).and()
				.assertThat().body("status", equalTo("available")).and().assertThat()
				.body("category.name", equalTo("Cat")).and().assertThat().body("tags[0].name", equalTo("dog"));
	}
	...
	public void i_send_a_get_request_by_status_to_valid_endpoint(String actualStatus) {
		response = given().accept(ContentType.JSON).queryParam("status", actualStatus).when().get(requestURL);
	...
	public void i_send_a_get_request_by_to_valid_endpoint(String expectedStatus) {
		response = given().accept(ContentType.JSON).queryParam("status", expectedStatus).when().get(requestURL);
	}
	...
	public void i_perform_get_request_to_url_with_pet_id(Integer petID) {
		response = given().accept(ContentType.JSON).when().get(requestURL + petID);
	...
	public void pet_id_is_pet_name_is_status_is(Integer petID, String petName, String petStatus) {
		response.then().assertThat().body("id", equalTo(petID)).and().assertThat().body("name", equalTo(petName)).and()
	...
	public void category_id_is_category_name_is(Integer categoryID, String categoryName) {
		response.then().assertThat().body("category.id", equalTo(categoryID)).and().assertThat().body("category.name",
				equalTo(categoryName));
	}

	@Then("Tags id is {int}, and tags name is {string}")
	public void tags_id_is_and_tags_name_is(Integer tagsID, String tagsName) {
		response.then().assertThat().body("tags[0].id", equalTo(tagsID)).and().assertThat()
				.body("tags[0].name", equalTo(tagsName)).and().assertThat().body("tags[1].id", equalTo(22002)).and()
				.assertThat().body("tags[1].name", equalTo("husky"));
	}
	...
	public void i_perform_delete_request_to_url_with_valid_id(Integer petID) {
		response = given().accept(ContentType.JSON).when().delete(requestURL + petID);
	...
	public void id_is_and_is(Integer petID, String petName) {
		response.then().assertThat().body("id", equalTo(petID)).and().assertThat().body("name", equalTo(petName));
	}
